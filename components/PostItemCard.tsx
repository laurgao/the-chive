...
export default function PostItemCard({post, randomNumberZeroToTwo = 2, wide}: {post: DatedObj<PostObj>, randomNumberZeroToTwo?: number, wide?: boolean}) {
    ...
    return (
        <div className={`${wide ? "max-w-60" : orientation == "flex-col" ? `${/*md:*/"max-w-30"}` : "flex-grow"} border border-transparent hover:border-gray-200 rounded-lg p-4 transition`}>
            <Link href={post.urlName.substr(0, 5) == "https"  ? post.urlName : isPostulate ? `https://postulate.us/@laura/p/${post.urlName}` : `/${post.urlName}`}>
                <a>
                    <div className={`flex ${orientation}`}>
                        {img && <div >
                            <img src={img} className={`border-left border-color-black ${orientation == "flex-col" ? "w-full" : "md:max-w-xs"} ${randomNumberZeroToTwo == 0 ? "mr-10" : randomNumberZeroToTwo == 1 && "ml-10"}`}/>
                        </div>}
